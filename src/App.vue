...
import { useDeedluma } from './useDeedluma'
import { CHAIN_ID, CONTRACT_ADDRESS } from './contract'

const deedluma = useDeedluma()
const { configured, wallet, token, native, treasury, totalSupply, treasuryBalance, treasuryNative, totalStaked, apr, paused,
  ...
  transact, questStatus, proposalStatus, votePercent, parseAmount } = deedluma
...
const samples = [
  { id: 'sample-1', sample: true, title: 'Read the Deedluma charter', reward: 12n * 10n ** 18n, detail: 'A sample welcome quest. Learn how rewards and stewardship work.' },
...
<div class="shell">
  ...
  <header class="top">
    <button class="logo" aria-label="Deedluma home" @click="go('home')"><span aria-hidden="true">&#10022;</span> DEEDLUMA</button>
  ...
  <main id="main">
    ...
    <section v-if="tab==='home'" class="hero"><div><span class="eyebrow">&#10022; FIELD NOTES / 01</span><h1>A public square<br><em>for useful work.</em></h1><p>Deedluma turns market curiosity into shared infrastructure. Complete open quests, stake conviction, and give every decision a visible trail.</p><div class="hero-actions"><button class="primary" @click="go('quests')">Explore quests &#8599;</button><button class="quiet" @click="go('learn')">Read the charter</button></div><div class="hero-footnote">COMMUNITY BUILT <span>/</span> TESTNET FIRST <span>/</span> OPEN CONTRACTS</div></div><div class="orb" aria-hidden="true"><div class="orbit one"></div><div class="orbit two"></div><div class="core">R<small>DLU / 01</small></div><span class="orb-note n1">OPEN BY DEFAULT</span><span class="orb-note n2">CONTRIBUTE / STEWARD</span></div></section>
    <section v-if="tab==='home'" class="home-grid"><article><span class="label">THE LOOP</span><h2>Contribute. Earn. Steward.</h2><p>Deedluma reward rules are public contract state. A contribution becomes a token balance, a position becomes a commitment, and a vote becomes a record.</p><div class="steps"><span>01</span><b>Discover an open quest</b><span>02</span><b>Build your DLU position</b><span>03</span><b>Shape the next Deedluma season</b></div></article><article class="quote"><span aria-hidden="true">&ldquo;</span><p>Make useful work easy to find. Make its rewards easy to verify.</p><small>Deedluma CHARTER / 2026</small><button class="quiet" @click="go('governance')">Enter governance &#8594;</button></article></section>
    ...
    <section v-if="tab==='governance'" class="page"><div class="page-head"><div><span class="eyebrow">Deedluma GOVERNANCE / 04</span><h1>Make a case.<br><em>Cast a weight.</em></h1><p>Publish a decision for discussion, then vote For or Against. These votes are advisory and do not automatically spend treasury funds.</p></div><button class="primary" :aria-expanded="showProposal" @click="showProposal=!showProposal">{{ showProposal ? 'Close editor' : '+ New proposal' }}</button></div>
      ...
      <div class="empty" v-if="configured && !reading && !visibleProposals.length">{{ readError ? 'Proposal data is unavailable. Use Refresh to retry.' : 'No proposals yet. Start the first Deedluma discussion.' }}</div>
